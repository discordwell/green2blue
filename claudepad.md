...
## 2026-06-19T00:00Z - Fix silent data loss for "00" international-prefix numbers (+9 tests)
- Clean tree, no WIP. Audited the conversion entry points (`converter/phone.py`, `converter/message_converter.py`, `parser/ndjson_parser.py`) — the least-touched-recently consequential path, since direction/overwrite bugs were already handled in the last two passes.
- **Bug (data loss, international users):** `normalize_phone` handled `+` and single trunk-zero prefixes but NOT the ITU-T `00` international access prefix used everywhere outside North America. A foreign contact stored as `0049170...` / `0044...` (extremely common on EU/Asia/Africa phones — it's how those regions dial and store international numbers) failed `PhoneNormalizationError`. For SMS that **silently skips the entire message** (`convert_messages` catches the error and increments `skipped_count`); for MMS it drops that participant. Reproduced: `00491701234567` (GB) → 0 messages, 1 skipped.
- **Fix:** after the `has_plus` branch and before the country-specific logic, `if digits.startswith("00"): return _validate_international(digits[2:], original)`. `00<cc><national>` is the spelled-out form of `+<cc><national>`; no national numbering plan assigns subscriber numbers beginning with `00`, so a leading `00` unambiguously means "international" regardless of `--country`. Reuses `_validate_international` (E.164 7-15 digit check), so too-short residue still raises.
- **Ordering verified safe:** short-code check (≤6 digits) and `has_plus` both run *before* the new branch, so `00`-short things stay short codes and `+`-numbers are untouched; single trunk-zero (`07788...` GB) doesn't match `00` so trunk handling is unaffected. Works for unknown countries too (placed before the `country not in COUNTRY_RULES` gate).
- **Tests (+9):** `TestInternationalAccessPrefix` (8 cases: UK/DE/FR forms, country-independence, formatting, unknown-country, parity with `+` form, single-zero-still-trunk, too-short-raises) + a converter-level regression (`test_intl_access_prefix_not_dropped`) proving the message is no longer dropped. Updated the `phone.py` module docstring.
- **Verification:** 682 tests pass (was 673, +9). ruff clean. Run with `source .venv/bin/activate && python -m pytest` (venv is a proper editable install). Not pushed (orchestrator pushes).

## 2026-06-18T13:07Z - Fix overwrite-mode "message kind" corruption (+2 tests)
- Clean tree, no WIP. Hunted bugs with parallel review agents over the least-audited consequential modules (`archive/`, `ios/sms_db.py`); verified findings myself before acting.
- **Bug (data integrity, overwrite mode):** `SMSDatabase._overwrite_message` UPDATEd only content columns (`text`, dates, direction flags, attributedBody/MSI). A "sacrifice" pool is *arbitrary existing rows* (`_load_sacrifice_messages` does no filtering), so if a sacrifice row was a tapback/reaction (`associated_message_type != 0`), an app/balloon message (`balloon_bundle_id`/`payload_data`), an audio message (`is_audio_message=1`), a system event (`item_type != 0`), or edited/unsent (`date_edited`/`date_retracted`), those markers survived the overwrite. The injected Android text then rendered as the **wrong kind** of message — e.g. a "loved" reaction to a now-missing message — even though text/date were correct. INSERT mode is unaffected (fresh rows default these to 0/NULL).
- **Fix:** added module constant `_OVERWRITE_RESET_COLUMNS` (24 cols → 0 or NULL, matching a fresh INSERT) and a schema-aware reset loop appended to the overwrite UPDATE's dynamic `opt_sets`/`opt_params`. CK metadata, ROWID, and GUID are deliberately excluded — overwrite mode preserves those on purpose. Guarded by `col in self._msg_schema` so older iOS sms.db schemas are unaffected.
- **Tests:** new `TestOverwriteKindNormalization` (2 tests): one seeds all 24 kind-columns with corrupt values and asserts each normalizes; one asserts the reset does NOT disturb preserved `guid`/`ck_*`. Verified the first FAILS without the fix (`associated_message_type 2000 != 0`).
- Other agent findings triaged but NOT acted on: archive group-clustering can drop a distinct same-titled/overlapping-group message that's byte-identical in the same second (MEDIUM, documented heuristic tradeoff — fix needs deeper merge-design work); dedup misses orphan messages lacking a chat_message_join (LOW, doesn't fire on normal workflow).
- Also corrected two stale/incomplete `ARCHITECTURE.md` sections: the Chat GUID format (groups are `any;+;chat<decimal>`, not the old `any;-;chat{sha256[:16]}` — code changed in a8ca10b, doc never updated) and the Overwrite Mode flow (now documents the kind-column reset).
- **Verification:** 673 tests pass (was 671, +2). ruff clean. Run with `source .venv/bin/activate && python -m pytest` (venv is now a proper editable install — the older "STALE non-editable install" note no longer applies). Not pushed (orchestrator pushes).

## 2026-06-17T21:30Z - Fix message-direction bug for OUTBOX/FAILED/QUEUED/DRAFT (+10 tests)
- **Bug (data integrity):** direction was inferred from `sms.type == 2` / `mms.msg_box == 2` (SENT only). Android's Telephony model also has OUTBOX(4)/FAILED(5)/QUEUED(6)/DRAFT(3) — all owner-authored. These were classified as **received**, so a user's own undelivered/queued/failed messages would land in iOS Messages as if the **contact** sent them. Only INBOX(1) is incoming.
- **Root cause was duplication:** the same direction rule was copied across 4 sites and had drifted. Inject converter (`message_converter._convert_sms/_convert_mms`) hardcoded `== 2`; the archive importer (`android_import`) and the review preview (`review.py`) each had their own `_sms_direction`/`_mms_direction` with `== 2` (re-exporting outgoing buckets as received via `unknown`→type 1); corpus bucketing (`corpus._classify_message`) used `type == 2`.
- **Fix — single source of truth in `models.py`:** added `SMS_OUTGOING_TYPES = {2,3,4,5,6}`, `MMS_OUTGOING_BOXES = {2,3,4}`, and canonical `android_sms_direction()`/`android_mms_direction()` (3-state incoming/outgoing/unknown). Unknown/vendor values still fall back to received (inject) / unknown (archive), preserving prior behavior for unrecognized codes. Rewired all 4 consumers; deleted the 3 duplicate helper copies.
- **DRAFT(3) decision:** treated as outgoing (`is_from_me=True`). A draft is owner-authored; the only alternatives were the strictly-wrong "received" or silent data loss. Minor wrinkle: a draft also gets `is_sent=True` (iOS has no draft concept) — acceptable vs. the direction flip.
- **Independent review subagent** verified the constants vs. the real Android Telephony provider and found the 2 missed sites (review.py, corpus.py), which this fix folds in. Confirmed `ios_import`/`render_verify` correctly derive direction from iOS `is_from_me` (out of scope).
- **Verification:** 671 tests pass (was 661, +10). Every new test proven to FAIL on the pre-fix logic (temporarily reverted each call site, watched it fail, restored). ruff clean. Run with `PYTHONPATH=src python -m pytest` (venv has a STALE non-editable install). Not pushed (orchestrator pushes).
