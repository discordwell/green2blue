...
def _create_reduced_sms_schema(conn: sqlite3.Connection) -> None:
    """Create an sms.db schema simulating an older iOS version (pre-iOS 18).

    Omits optional columns: message_summary_info, destination_caller_id,
    ck_chat_id, sr_ck_sync_state, preview_generation_state, original_guid.
    """
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS handle (
            ROWID INTEGER PRIMARY KEY AUTOINCREMENT,
...
@pytest.fixture
def reduced_schema_sms_db(tmp_dir: Path) -> Path:
    """Create an sms.db with a reduced schema (older iOS version)."""
    db_path = tmp_dir / "reduced_sms.db"
    conn = sqlite3.connect(db_path)
    _create_reduced_sms_schema(conn)
    conn.close()
    return db_path
